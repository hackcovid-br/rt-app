...
# COVID-19 Confirmed vs. New cases`
  ...
  main.variable(observer()).define(["md"], function(md) {
    return md`
A recreation of the visualization by [Aatish Bhatia](https://aatishb.com/covidtrends/) & [minutephysics](https://www.youtube.com/watch?v=54XLXg4fYsc).
...
## Visualization`
  ...
  main
    .variable(observer("viewof curr_date"))
    .define("viewof curr_date", ["Scrubber", "d3"], function(Scrubber, d3) {
      return Scrubber(d3.timeDays(new Date(2020, 0, 22), new Date()), {
        autoplay: true,
        loop: false,
        alternate: false,
        initial: 0,
  ...
  main
    .variable(observer("curr_date"))
    .define("curr_date", ["Generators", "viewof curr_date"], (G, _) =>
      G.input(_)
    )
  main
    ...
    .define(
      "chart",
      ["d3", "params", "draw_axes", "draw_trajectories", "draw_date"],
      function(d3, params, draw_axes, draw_trajectories, draw_date) {
        const svg = d3
          .create("svg")
  ...
  main
    ...
    .define("onMouseOver", ["d3", "code_to_name", "x", "y"], function(
      d3,
      code_to_name,
      x,
      y
      ...
      return function onMouseOver() {
        ...
        d3.select("#fullname")
          .text(code_to_name(country_code))
  ...
  main
    .variable(observer("draw_date"))
    .define("draw_date", ["d3", "curr_date", "x", "params", "y"], function(
      d3,
      curr_date,
      x,
      params,
      ...
      return function draw_date(svg) {
        svg
          .append("g") // Date
          .append("text")
          .text(d3.timeFormat("%b %d")(curr_date))
          ...
          .attr("y", y(params.ymin))
      }
    })
  main
    .variable(observer("draw_axes"))
    .define("draw_axes", ["xAxis", "yAxis", "grid"], function(
      xAxis,
      yAxis,
      grid
    ) {
...
## Visualization elements`
      ...
      function(
        ...
        return function draw_trajectories(svg) {
          ...
          svg
            ...
            .selectAll("text")
            .data(
              viz_data.filter(d => countries_to_label.includes(d.country_code))
            )
            .join("text")
  ...
  main
    ...
    .define("get_path", ["d3", "x", "y"], function(d3, x, y) {
      return function get_path(d) {
        const l = d3.line()
        return l(
          d.filter(d => d[1] > 10 && d[2] > 10).map(d => [x(d[1]), y(d[2])])
        )
      }
    })
  main
    .variable(observer("grid"))
    .define("grid", ["x", "params", "y"], function(x, params, y) {
      return g =>
        g
  ...
  main
    ...
    .define("xAxis", ["params", "d3", "x", "y_type"], function(
      params,
      d3,
      x,
      y_type
...
## Visualization parameters`
  ...
  main
    ...
    .define("color", ["d3", "cntry_data_raw", "us_state_data"], function(
      d3,
      cntry_data_raw,
      us_state_data
    ) {
      return d3
        .scaleOrdinal(
          cntry_data_raw.concat(us_state_data).map(d => d.region),
          d3.schemeCategory10
        )
        .unknown("black")
    })
...
## Data`
  ...
  main
    ...
    .define("code_to_name", ["data"], function(data) {
      return function code_to_name(country_code) {
        return data.find(d => d.country_code === country_code).country_name
      }
    })
  main
    .variable(observer("viz_data"))
    .define("viz_data", ["data", "curr_date"], function(data, curr_date) {
  ...
  main
    .variable(observer("last_index"))
    .define("last_index", ["data"], function(data) {
      return data[0].confirmed.length - 1
    })
  main
    ...
    .define(
      ...
      function(
        ...
        return cntry_data_raw
          .concat(us_state_data)
  ...
  main
    .variable(observer("us_state_data"))
    .define("us_state_data", ["us_state_data_raw"], function(
      us_state_data_raw
    ) {
      return us_state_data_raw.map(d => ({
        country_code: d.code,
        country_name: d.name,
        region: "US States",
  ...
  main
    .variable(observer("us_state_data_raw"))
    .define("us_state_data_raw", ["d3"], function(d3) {
      return d3.json(
        "https://raw.githubusercontent.com/covid19-data/covid19-data/master/output/us_state_nyt.json"
      )
    })
  main
    .variable(observer("cntry_data_raw"))
    .define("cntry_data_raw", ["d3"], function(d3) {
      return d3.json(
        "https://raw.githubusercontent.com/covid19-data/covid19-data/master/output/cntry_stat_owid.json"
      )
    })
  main.variable(observer()).define(["md"], function(md) {
    return md`

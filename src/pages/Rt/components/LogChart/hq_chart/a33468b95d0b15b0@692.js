...
Then call the legend function as shown below.`
  ...
  main.variable(observer()).define(["legend", "d3"], function(legend, d3) {
    return legend({
...
How about swatches for ordinal color scales? Both variable-width swatches and [column layout](https://developer.mozilla.org/en-US/docs/Web/CSS/columns) are supported.`
  ...
  main.variable(observer()).define(["swatches", "d3"], function(swatches, d3) {
    return swatches({
      color: d3.scaleOrdinal(
...
## Implementation`
  ...
  main
    ...
    .define("legend", ["d3", "ramp"], function(d3, ramp) {
  ...
  main
    .variable(observer("swatches"))
    .define("swatches", ["DOM", "html", "entity"], function(DOM, html, entity) {
  ...
  main.variable(observer("entity")).define("entity", function() {
    return function entity(character) {
      return `&#${character.charCodeAt(0).toString()};`
    }
  })
  main.variable(observer("ramp")).define("ramp", ["DOM"], function(DOM) {
    return function ramp(color, n = 256) {

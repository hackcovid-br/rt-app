...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["md"], function (md) {
    return (
      md`# COVID-19 Casos e Mortes no Brasil`
    )
  });
  main.variable(observer()).define(["md"], function (md) {
    return (
      md`## Evolução por Estado`
    )
  });
  ...
  main.variable(observer("viewof pivot_n")).define("viewof pivot_n", ["slider", "pivot_values"], function (slider, pivot_values) {
    return (
      slider({
  ...
  main.variable(observer("pivot_n")).define("pivot_n", ["Generators", "viewof pivot_n"], (G, _) => G.input(_));
  main.variable(observer("viewof num_days")).define("viewof num_days", ["slider"], function (slider) {
    return (
      slider({
  ...
  main.variable(observer("num_days")).define("num_days", ["Generators", "viewof num_days"], (G, _) => G.input(_));
  main.variable(observer("viewof doubling_periods")).define("viewof doubling_periods", ["checkbox"], function (checkbox) {
    return (
      checkbox({
  ...
  main.variable(observer("doubling_periods")).define("doubling_periods", ["Generators", "viewof doubling_periods"], (G, _) => G.input(_));
  ...
  main.variable(observer("chart")).define("chart", ["d3", "params", "draw_axes", "draw_doubling_lines", "draw_trajectories"], function (d3, params, draw_axes, draw_doubling_lines, draw_trajectories) {
  ...
  main.variable(observer("onMouseOver")).define("onMouseOver", ["d3", "code_to_name", "x", "params", "y"], function (d3, code_to_name, x, params, y) {
    return (
  ...
  main.variable(observer("draw_axes")).define("draw_axes", ["xAxis", "yAxis"], function (xAxis, yAxis) {
    return (
  ...
  main.variable(observer("draw_trajectories")).define("draw_trajectories", ["viz_data", "color", "params", "get_path", "onMouseOver", "onMouseOut", "x", "y", "d3"], function (viz_data, color, params, get_path, onMouseOver, onMouseOut, x, y, d3) {
    return (
  ...
  main.variable(observer("draw_doubling_lines")).define("draw_doubling_lines", ["doubling_periods", "d3", "x", "y", "params", "doubling_period_labels", "get_text_rotation_angle"], function (doubling_periods, d3, x, y, params, doubling_period_labels, get_text_rotation_angle) {
    return (
      function draw_doubling_lines(svg) {
        ...
        svg
          ...
          .join("text")
          .text(d =>
            d.period > 1 ? `Dobrando a cada ${d.period} dias` : "Dobrando todo dia"
          )
          .style("text-anchor", "end")
          .attr("font-size", 10)
          .attr("font-family", "Helvetica, sans-serif")
          ...
          .attr(
            "transform",
            d => `rotate(${get_text_rotation_angle(d.period)} ${x(d.x)} ${y(d.y)})`
          )
          .attr("x", d => x(d.x))
          .attr("y", d => y(d.y));
      }
  ...
  main.variable(observer("doubling_period_labels")).define("doubling_period_labels", ["doubling_periods", "params"], function (doubling_periods, params) {
    return (
      doubling_periods.map(function (d) {
        d = Number(d);
  ...
  main.variable(observer("params")).define("params", ["pivot", "num_days"], function (pivot, num_days) {
    return (
      {
        width: 600,
        height: 400,
  ...
  main.variable(observer("xAxis")).define("xAxis", ["params", "d3", "x", "width"], function (params, d3, x, width) {
    return (
      g =>
        g
  ...
  main.variable(observer("x")).define("x", ["d3", "params"], function (d3, params) {
    return (
      d3.scaleLinear().domain([params.xmin, params.xmax])
  ...
  main.variable(observer("get_text_rotation_angle")).define("get_text_rotation_angle", ["y", "params", "x"], function (y, params, x) {
    return (
  ...
  main.variable(observer("code_to_name")).define("code_to_name", ["data"], function (data) {
    return (
      function code_to_name(state_code) {
        return data.find(d => d.state_code === state_code).state_name;
      }
    )
  });
  main.variable(observer("viz_data")).define("viz_data", ["data", "selected_regions", "y_type", "pivot", "pivot_bisector"], function (data, selected_regions, y_type, pivot, pivot_bisector) {
    return (
      data
  ...
  main.variable(observer("pivot_bisector")).define("pivot_bisector", ["d3"], function (d3) {
    return (
      d3.bisector(d => d[1])
    )
  });
  ...
  main.variable(observer("pivot")).define("pivot", ["pivot_values", "pivot_n"], function (pivot_values, pivot_n) {
    return (
      pivot_values[pivot_n]
    )
  });
  main.variable(observer("pivot_values")).define("pivot_values", function () {
    return (
      [1, 5, 10, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 13000, 15000, 20000, 50000, 100000]
    )
  });
  main.variable(observer("data")).define("data", ["data_raw", "past_week"], function (data_raw, past_week) {
    return (
  ...
  main.variable(observer("past_week")).define("past_week", ["d3"], function (d3) {
    return (
